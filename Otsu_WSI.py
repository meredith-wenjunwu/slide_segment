"""
Segmentation of dermis , epidermis, and background using otsu thresholding
...
def Otsu_thresh(args):
    ...
    binary_or_multi = args.binary_or_multi
    ...
    def threshold_output(filename):
        ...
        if binary_or_multi == "binary":
            ...
            out_image = np.digitize(Img, bins=thresholds)
    ...
    if choice == "category":
        for category in categories:
            for subpath in sorted(glob(os.path.join(in_path , category) + "/*/")):
                for filename in sorted(glob(subpath + "*.jpg")):
                    print(filename)
                    out_dir = subpath.replace(out_path,in_path) 
                    ...
                    out_name = os.path.join(out_dir , image_name)
        ...
        for subpath in sorted(glob(os.path.join(in_path) + "/*/")):
            for filename in sorted(glob(subpath + "*.jpg")):
                print(filename)
                out_dir = subpath.replace(out_path,in_path) 
        ...
        image_name = filename.split("/")[-1]                  
        out_name = os.path.join(out_path , os.path.splitext(image_name)[0] +
            '.png')
        Image.fromarray(out_image).save(out_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--binary_or_multi", default="binary", choices = ["binary","multi"],
                    help="binary thresholding or multi thresholding")   
